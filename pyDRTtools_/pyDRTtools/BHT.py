# -*- coding: utf-8 -*-
__authors__ = 'Francesco Ciucci, Ting Hei Wan, Adeleke Maradesa'
...
def assemble_A_re_bht(freq_vec, tau_vec, impedance=True):
    """
    Computes the discretization matrix, A_re, for the real part of the impedance or admittance.
    ...
    Outputs:
    - out_M: the discretization matrix for the real part of the impedance
    """
    ...
    for p in range(0, N_freqs):
        for q in range(0, N_taus):
            if q == 0:
                out_A_re[p, q + 1] = 0.5 / (1 + (omega_vec[p] * tau_vec[q]) ** 2) * log(
                    tau_vec[q + 1] / tau_vec[q]) if impedance else 0.5 * (omega_vec[p] ** 2 * tau_vec[q]) / (
                        1 + (omega_vec[p] * tau_vec[q]) ** 2) * log(tau_vec[q + 1] / tau_vec[q])
            elif q == N_taus - 1:
                out_A_re[p, q + 1] = 0.5 / (1 + (omega_vec[p] * tau_vec[q]) ** 2) * log(
                    tau_vec[q] / tau_vec[q - 1]) if impedance else 0.5 * (omega_vec[p] ** 2 * tau_vec[q]) / (
                        1 + (omega_vec[p] * tau_vec[q]) ** 2) * log(tau_vec[q] / tau_vec[q - 1])
            else:
                out_A_re[p, q + 1] = 0.5 / (1 + (omega_vec[p] * tau_vec[q]) ** 2) * log(
                    tau_vec[q + 1] / tau_vec[q - 1]) if impedance else 0.5 * (omega_vec[p] ** 2 * tau_vec[q]) / (
...
def assemble_A_im_bht(freq_vec, tau_vec, impedance=True):
    """
    Computes the discretization matrix, A_im, for the imaginary part of the impedance or admittance in the BHT run.
    Inputs:
    - tau_vec: vector of timescales
    - freq_vec: vector of frequency

    Outputs:
    - out_M: the discretization matrix for the imaginary part of the impedance
    """
    omega_vec = 2. * pi * freq_vec
    N_freqs = freq_vec.size
    N_taus = tau_vec.size
    ...
    out_A_im[:, 0] = -omega_vec  # Note the negative sign

    for p in range(0, N_freqs):
        for q in range(0, N_taus):
            if q == 0:
                out_A_im[p, q + 1] = -0.5 * (omega_vec[p] * tau_vec[q]) / (
                        1 + (omega_vec[p] * tau_vec[q]) ** 2) * log(tau_vec[q + 1] / tau_vec[q]) if impedance else \
                                       0.5 * omega_vec[p] / (1 + (omega_vec[p] * tau_vec[q]) ** 2) * log(
                                           tau_vec[q + 1] / tau_vec[q])
            elif q == N_taus - 1:
                out_A_im[p, q + 1] = -0.5 * (omega_vec[p] * tau_vec[q]) / (
                        1 + (omega_vec[p] * tau_vec[q]) ** 2) * log(tau_vec[q] / tau_vec[q - 1]) if impedance else \
                                       0.5 * omega_vec[p] / (1 + (omega_vec[p] * tau_vec[q]) ** 2) * log(
                                           tau_vec[q] / tau_vec[q - 1])
            else:
                out_A_im[p, q + 1] = -0.5 * (omega_vec[p] * tau_vec[q]) / (
                        1 + (omega_vec[p] * tau_vec[q]) ** 2) * log(tau_vec[q + 1] / tau_vec[q - 1]) if impedance else \
                                       0.5 * omega_vec[p] / (1 + (omega_vec[p] * tau_vec[q]) ** 2) * log(
                                           tau_vec[q + 1] / tau_vec[q - 1])

    return out_A_im


def assemble_M_1_bht(tau_vec):
    """
    Computes the matrix, M, of the inner products of the first derivatives of the RBF functions used in the expansion.
    For the BHT run.
    ...
    # Get the number of timescales
    N_taus = len(tau_vec)
    
    # Initialize the output matrix M with zeros
    out_M = np.zeros((N_taus - 2, N_taus + 1))

    # Iterate over the collocation points (p)
    for p in range(0, N_taus - 2):
        # Compute the distance between neighboring collocation points
...
def assemble_M_2_bht(tau_vec):
    """
    Computes the matrix, M, of the inner products of the second derivatives .
    Input:
    - tau_vec: vector of timescales
...
def compute_A_H_re(freq_vec, tau_vec):
    
    out_A_re = assemble_A_re_bht(freq_vec, tau_vec, impedance = True)    
    out_A_H_re = out_A_re[:,1:]
    
    return out_A_H_re


def compute_A_H_im(freq_vec, tau_vec):
    
    out_A_im = assemble_A_im_bht(freq_vec, tau_vec, impedance=True) 
...
def HT_est(theta_0, Z_exp, freq_vec, tau_vec, Dn='D2'):
    ...
    N_freqs = freq_vec.size                 ## , data_flag='impedance'
    N_taus = tau_vec.size

    # compute the matrix $A = A_re + i A_im$
    A_re = assemble_A_re_bht(freq_vec, tau_vec, impedance=True)
    #compute_A_re(freq_vec, tau_vec, epsilon=0, rbf_type='PWL', flag1='BHT', flag2=data_flag)
    A_im = assemble_A_im_bht(freq_vec, tau_vec, impedance=True)
    
    #compute_A_im(freq_vec, tau_vec, epsilon=0, rbf_type='PWL', flag1='BHT', flag2=data_flag)
    
    # as well as the ones used for the Hilbert transform
    A_H_re = compute_A_H_re(freq_vec, tau_vec)
    A_H_im = compute_A_H_im(freq_vec, tau_vec)

    # compute the matrix L
    if Dn == 'D1': # first-order differentiation matrix
        L = assemble_M_1_bht(tau_vec)
        
    else: # second-order differentiation matrix
        L = assemble_M_2_bht(tau_vec)

...
#import parameter
# Part 1: Investigate nearest positive definite
...
# Part 2: Discretization of the DRT problem and recovery of the DRT using ridge regression
...
def optimal_lambda(A_re, A_im, Z_re, Z_im, M, log_lambda_0, cv_type):
    ...
    # GCV method
    if cv_type == 'GCV': 
        res = minimize(parameter.compute_GCV, log_lambda_0, args=(A_re, A_im, Z_re, Z_im, M), options={'disp': True, 'maxiter': 2000}, bounds = bnds, method = 'SLSQP')
        print('GCV')
    
    # mGCV method
    elif cv_type == 'mGCV': 
        res = minimize(parameter.compute_mGCV, log_lambda_0, args=(A_re, A_im, Z_re, Z_im, M), options={'disp': True, 'maxiter': 2000}, bounds = bnds, method = 'SLSQP')
        print('mGCV')
        
    # rGCV method
    elif cv_type == 'rGCV': 
        res = minimize(parameter.compute_rGCV, log_lambda_0, args=(A_re, A_im, Z_re, Z_im, M), options={'disp': True, 'maxiter': 2000}, bounds = bnds, method = 'SLSQP')
        print('rGCV')  
    
    # L-curve method
    elif cv_type == 'LC':
        res = minimize(parameter.compute_LC, log_lambda_0, args=(A_re, A_im, Z_re, Z_im, M), options={'disp': True, 'maxiter': 2000}, bounds = bnds, method = 'SLSQP')
        print('LC')
    
    # re-im discrepancy
    elif cv_type == 're-im':
        res = minimize(parameter.compute_re_im_cv, log_lambda_0, args=(A_re, A_im, Z_re, Z_im, M), options={'disp': True, 'maxiter': 2000}, bounds = bnds, method = 'SLSQP')
        print('re-im')
        
    # k-fold 
    elif cv_type == 'kf':  
        res = minimize(parameter.compute_kf_cv, log_lambda_0, args=(A_re, A_im, Z_re, Z_im, M), options={'disp': True, 'maxiter': 2000}, bounds = bnds, method = 'SLSQP')
        print('kf') 
